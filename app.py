...
def chart_js(html_str, height=300):
    st.components.v1.html(
        f"""<div class="chart-wrap" style="background:#0b1e2d;border:1px solid rgba(0,229,200,.15);border-radius:1rem;padding:1.4rem">
        {html_str}
        </div>
...
# ════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        meal_cards([
            ...
            ("E","Chicken Salad Bowl (lighter)",["200g chicken breast (shredded, grilled)","2 whole eggs + 1 egg white","100g cucumber + 50g capsicum + 1 tomato (raw)","150g curd","1 banana"],"~600 kcal","~78g protein","Low ✅","Highest fibre option"),
        ])
    with tab2:
...
# ════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="stag">Schedule</div><div class="stitle">Weekly Rotation</div>'
                '<p class="ssub">Mutton & Prawns removed · All uric-acid safe · Palak 4× per week for iron</p>', unsafe_allow_html=True)
    ...
    cols = st.columns(7)
    for col, (day, m1, sn, dn, today) in zip(cols, weekly):
        with col:
    ...
    st.markdown('<br><div class="info-box"><strong>🌱 Palak Strategy</strong> Mon, Thu, Sat, Sun = 4 days/week. Directly addresses confirmed Iron Deficiency Anaemia.</div>', unsafe_allow_html=True)
    st.markdown('<div class="info-box"><strong>🅒 Wednesday Paneer Swap</strong> Permanently replaces Mutton Day until uric acid normalises (< 6.0 mg/dL on retest in late May).</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("<div style='font-size:.73rem;font-weight:700;letter-spacing:.1em;color:#00e5c8;text-transform:uppercase;margin-bottom:.55rem'>✅ ADDED</div>", unsafe_allow_html=True)
        for food, reason, when in [
            ("Palak 100g","Iron deficiency confirmed","3–4×/week"),
            ("Banana (daily)","Potassium, uric acid","Every day"),
            ("Lemon warm water","Citric acid flushes UA","On waking"),
            ("Garlic (daily)","Liver + anti-inflammatory","All cooking"),
            ("Walnuts 2/day","Omega-3 plant source","Daily"),
        ]:
